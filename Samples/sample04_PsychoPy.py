...
#tracker.setCalibrationTargetStimulus(psychopy.visual.Rect(win,width=100,height=100,units='pix'))
calstim = [psychopy.visual.Rect(win,width=5,height=5,units='pix',lineColor=(1,0,0)),psychopy.visual.Rect(win,width=5,height=5,units='pix')]
tracker.setCalibrationTargetStimulus(calstim)

def callback(self, t, index, targetPos, currentPos):
    if t<1.0:
        self.caltarget[0].setSize(((10-9*t)*10,(10-9*t)*10))
    else:
        self.caltarget[0].setSize((10,10))
    print (t,targetPos,currentPos)
